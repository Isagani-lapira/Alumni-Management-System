$(document).ready(function () {
  ...
  $('a[data-link="forms"]').on("click", function () {
    // refreshList();
    setTimeout(function () {
      setHandlers();
      console.log('refreshed the handlers of "forms"');
    }, 500);
  });
  ...
  function setHandlers() {
    ...
    $("#previewFormBtn").on("click", function () {
      ...
      $("#previewFormBtn").addClass("hidden");

      // store the data that retrieved
      retrieveCategory().then((categoryID) => {
    ...
    $("#backFromPreviewBtn").on("click", function () {
      ...
      $("#previewFormBtn").removeClass("hidden");
        ...
        $.ajax({
          ...
          success: (response) => {
            if (response.result == "Success") {
              const categoryID = response.categoryID.map(
                (category) => category
              ); // Copy the category IDs
              resolve(categoryID);
            }
          },
        });
    ...
    function displayQuestion(categoryName, questionSets, container) {
      ...
      questionSets.forEach((value) => {
        ...
        const questionWrapper = $("<div>").addClass("w-full");
        ...
        const selectWrapper = $("<select>").addClass(
          "w-1/2 border border-gray-300 p-3 rounded-md"
        );
        ...
        if (inputType !== "Input") {
          value.choices.forEach((choice) => {
            ...
            // radio type question
            if (inputType === "Radio") {
              const choiceWrapper = $("<div>").addClass(
            ...
            // dropdown question
              ...
              const option = $("<option>")
                .val(choiceID)
                .text(choice_text)
                .attr("data-sectionchoice", hasSectionQuestion);
