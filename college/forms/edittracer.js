$(document).ready(function () {
  ...
  function retrieveCategory(formIDValue) {
    ...
    $("#nextpage").on("click", function () {
      ...
      $("#categoryContainer, #paginationWrapper, #noteCategory").addClass(
        "hidden"
      );
  ...
  function addCategory(categoryID, categoryName) {
    ...
    const inputField = $("<input>")
      ...
      .addClass(
        "border-b border-gray-400 py-2 px-2 outline-none w-full categoryField"
      )
      .on("change", function () {
        displaySavedChanges();
    ...
    const removeBtn = $(
      '<iconify-icon icon="healthicons:x" style="color: #afafaf;" width="24" height="24"></iconify-icon>'
    ).on("click", function () {
      displaySavedChanges();
  ...
  function displayQuestion(
    ...
    const categoryWrapper = $("<div>").addClass(
      "rounded-lg mx-auto mb-6 p-1 relative questionCategoryWrapper"
    );

    const headerCategory = $("<header>")
      .addClass(
        "w-full text-center py-5 font-bold text-lg text-white bg-darkAccent rounded-t-lg"
      )
      .text(categoryName);

    const addNewQuestion = $(
      '<iconify-icon class="p-3 rounded-md hidden center-shadow h-max absolute top-1 right-1" icon="gala:add" style="color: #AFAFAF;" width="24" height="24"></iconify-icon>'
    ).on("click", function () {
    ...
    const bodyCategory = $("<div>").addClass("p-3 flex flex-col gap-2");
    ...
    questionSets.forEach((questionData) => {
      ...
      const questionWrapper = $("<div>").addClass(
        "relative center-shadow w-4/5 mx-auto"
      );

      const questionName = $("<input>")
        .addClass(
          "text-center font-bold center-shadow py-3 w-full rounded-t-md"
        )
        .val(questionTxt);
      ...
      if (!isQuestion) {
        ...
        questionName
          .removeClass("bg-accent text-white")
          .addClass("text-gray-500 border-b border-gray-400 border-t-4");
      }
      ...
      choices.forEach((choice) => {
        ...
        const wrapper = $("<div>").addClass(
          "flex items-center justify-between mb-2 wrapperChoices"
        );

        //mark up for input field with remove button
        const inputField = $("<input>")
          .addClass(
            "border-b border-gray-400 py-2 px-2 outline-none w-full categoryField text-gray-500"
          )
          .val(choice_text)
          .on("change", function () {
            displaySavedChanges();
        ...
        const sectionBtn = $(
          '<iconify-icon icon="uit:web-section-alt" class="p-2" style="color: #afafaf;" width="20" height="20"></iconify-icon>'
        )
        ...
        if (questionType != "Input") questionBody.append(wrapper);

        if (isQuestion) sectionBtn.addClass("hidden");
      });

      const removeQuestion = $(
        '<iconify-icon icon="octicon:trash-24" class="p-2 rounded-md center-shadow h-max remove-question absolute top-0 -right-12" style="color: #afafaf;" width="24" height="24"></iconify-icon>'
      ).on("click", function () {
