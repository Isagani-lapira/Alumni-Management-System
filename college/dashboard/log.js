$(document).ready(function () {
  ...
  function createActivities(
    ...
    const actionWrapper = $("<div>").addClass(
      "flex justify-stretch actionWrapper items-center"
    );
    ...
    const content = $("<div>").addClass("text-sm ms-2 font-extralight");

    const detailsWrapper = $("<div>").addClass("flex gap-2 items-center");
    const college = $("<span>")
      .addClass("text-gray-700 font-bold text-lg")
      .text(colCode);
    const detailsMsg = $("<p>").addClass("text-gray-500").text(details);
    ...
    const time = $("<span>").addClass("text-gray-500 text-xs").text(timestamp);
  ...
  $("#logCollege").on("change", function () {
    college = $(this).val();
    if (college != "") getFilteredDateAction(startDate, endDate, college);
    else {
      offset = 0;
      $("#logHistoryModal").removeClass("hidden");
      $("#logList").find(".actionWrapper").remove(); //remove the previously retrieve logs (not duplicate)
  ...
  $("#printLogsBtn").on("click", function () {
    ...
    window.open(
      "logtemplate.html?data=" + encodeURIComponent(jsonString),
      "_blank"
    );
  });
});
