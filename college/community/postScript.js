function handleEventListeners() {
  ...
  // let date = getFormattedDate(date);
  // let date = "";
  let date_posted = $("<p>").addClass("text-xs text-gray-500 my-2").text(date);
  ...
  function filterTable() {
    ...
    if (collgeFilter !== "" && startDateFilter === "" && endDateFilter === "")
      //college filtering
      ...
      table
        .column(3)
        .search(startDateFilter + "|" + endDateFilter, true, false, true)
        .draw();
    else if (
      collgeFilter !== "" &&
      startDateFilter !== "" &&
  ...
  function viewingOfPost(postID, name, accUN, description, images, likes) {
    ...
    images.forEach((image, index) => {
      ...
      const img = $("<img>")
        .addClass(
          "absolute inset-0 left-0 right-0 top-0 bottom-0 m-auto object-contain"
        )
        .attr("src", format)
        ...
        if (parsedResponse.result == "Success") {
          ...
          for (let i = 0; i < length; i++) {
            ...
            let imgProfile = $("<img>")
              ...
              .attr("src", img);
            let commentDescript = $("<div>").addClass(
              "bg-gray-300 rounded-md p-3 flex-grow text-sm flex flex-col gap-1 text-greyish_black"
            );
          ...
          let noCommentMsg = $("<p>")
            .addClass("text-gray-500 text-center")
            .text("No available comment");
  ...
  function displayPostToProfile(
    ...
    let fullnameElement = $("<p>")
      .addClass("font-bold text-greyish_black")
      .text(fullname);
    let usernameElement = $("<p>")
      .addClass("text-gray-400 text-xs")
      .text(username);
    ...
    if (images.length > 0) {
      //set up the swiper
      swiperContainer = $("<div>").addClass(
        "swiper relative profilePostSwiper w-full h-80 bg-black rounded-md cursor-pointer overflow-x-hidden"
      );
      ...
      images.forEach((value) => {
        ...
        //create slider
        const swiperSlider = $("<div>").addClass(
          "swiper-slide h-full flex justify-center"
        );
        const swiperImg = $("<img>")
      ...
      swiperContainer.on("click", function (event) {
        ...
        if (
          !$(event.target).hasClass("swiper-button-prev") &&
          !$(event.target).hasClass("swiper-button-next")
        ) {
          $("#modalPost").removeClass("hidden");
          viewingOfPost1(
            postID,
    ...
    // dito
    if (isLiked)
      heartIcon = heartIcon.html(
        '<iconify-icon icon="mdi:heart" style="color: #ed1d24;" width="20" height="20"></iconify-icon>'
      );
    else
      heartIcon = heartIcon.html(
        '<iconify-icon icon="mdi:heart-outline" style="color: #626262;" width="20" height="20"></iconify-icon>'
      );

    let interactionContainer = $("<div>").addClass(
      "border-t border-gray-400 p-2 flex items-center justify-between"
    );
    heartIcon
    ...
    let leftContainer = $("<div>")
      ...
      .append(heartIcon, likesElement, commentIcon, commentElement);
  ...
  function viewingOfPost1(
    ...
    images.forEach((image, index) => {
      ...
      const item = $("<div>")
        .addClass(
          "relative duration-700 ease-in-out h-full flex justify-center items-center"
        )
        .attr("data-carousel-item", "")
        .attr("id", imageName);
  ...
  function displayCommunityPost(
    ...
    if (images.length > 0) {
      //set up the swiper
      swiperContainer = $("<div>").addClass(
        "swiper communitySwiper relative  w-full h-80 bg-black rounded-md cursor-pointer overflow-x-hidden"
      );
      const swiperWrapper = $("<div>").addClass("swiper-wrapper h-full");
    ...
    //report
    let reportWrapper = $("<div>").addClass(
      "flex flex-col gap-2 text-xs flex-wrap text-gray-400 py-2 text-end"
    );
    let nudityCount = 0;
    let violenceCount = 0;
    ...
    if (report.response !== "Nothing") {
      ...
      let spanNudity = $("<span>").text(nudityCount + " Nudity");
      let spanViolence = $("<span>").text(violenceCount + " Violence");
      let spanTerror = $("<span>").text(terrorismCount + " Terrorism");
      let spanHS = $("<span>").text(falseInfoCount + " Hate Speech");
      let spanFalseInfo = $("<span>").text(suicideCount + " False Info");
      let spanHrass = $("<span>").text(harassmentCount + " Harassment");
      let spanSOS = $("<span>").text(speechCount + " SOS");

      reportWrapper.append(
        spanNudity,
        spanViolence,
        spanTerror,
        spanHS,
  ...
  $("#communityContainer").on("scroll", function () {
    ...
    if (
      scrollPosition + containerHeight >= contentHeight - scrollThreshold &&
      lengthRetrieved == 10
    ) {
      getCommunityPost();
    }
...
$(document).ready(function () {
  ...
  $('a[data-link="community"]').on("click", function () {
    // refreshList();
    setTimeout(function () {
