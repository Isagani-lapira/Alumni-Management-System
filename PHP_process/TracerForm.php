...
class TracerForm
{
    ...
    public function get_filtered_college_all_answered($deploymentID, $colCode)
    {
        $smt = $this->conn->prepare("SELECT
        P.personID,
    ...
    LEFT JOIN
        ...
        WHERE
        A.tracer_deployID = ? AND AL.colCode = ?; ");

        $smt->bind_param("sss", $deploymentID, $deploymentID, $colCode);
        $smt->execute();
        $result = $smt->get_result();
        $data = array();
        while ($row = $result->fetch_assoc()) {
