$(document).ready(function () {
  ...
  function categoryQuestion(answerID, categoryID) {
    ...
    if (categoryID == "5fcdbb93760ce2f85bb62e5ca4f90")
      progressBar.css({ width: "100%" }); //last category will be 100%
    $.ajax({
      ...
      success: (response) => {
        ...
        const navigationWrapper = $("<div>").addClass(
        ...
        const nextBtn = $("<button>")
          .addClass(
            "px-3 py-2 rounded-md bg-accent hover:bg-darkAccent text-white font-bold nextBtnQuestion"
          )
          .text("Next")
          .on("click", function () {
            ...
            if (
              areQuestionAnswered() &&
              isCompleted &&
              haveCheckedInCheckBox &&
              haveSelectedDropDown
            ) {
              count++;
              $(".questions").empty();
        ...
        const submitBtn = $("<button>")
          .addClass(
            "px-3 py-2 rounded-md bg-green-400 hover:bg-green-500 text-white font-bold hidden submitQuestionnaire"
          )
          .text("Submit")
          .on("click", function () {
            let isCompleted = true;
        ...
        if (count > 0) navigationWrapper.append(backBtn);
  ...
  async function displayQuestion(
    ...
    // wrapper container
    const questionWrapper = $("<div>").addClass(
      "rounded-md userQuestionTracer"
    );

    // question name
    const question = $("<h3>").addClass("text-lg font-bold").text(questionTxt);

    if (containerRoot == tracerQuestionWrapper)
    ...
    try {
      ...
      const questionBody = $("<div>").addClass("flex flex-col gap-2 py-2");
      ...
      if (inputType === "Input") {
        ...
        let questionType = $("<input>")
          .addClass(
            "border-b border-gray-400 p-2 w-full lg:w-1/2 rounded-b-md outline-none userinputData "
          )
          .attr("type", "text")
        ...
        const select = $("<select>").addClass(
          "w-1/2 px-2 py-4 outline-none border border-gray-400 rounded-lg dropdownQuestion"
        );
        const defaultOption = $(
        ...
        choices.forEach((choice) => {
          ...
          if (inputType === "Radio") {
            const inputFieldWrapper = $("<div>").addClass(
              "flex justify-start items-center gap-2 w-1/2 mt-3"
            );
            ...
            const choiceVal = $("<input>")
              ...
              .on("click", function () {
                ...
                if (choiceID == "ed7f8ed8d45c72df2d9d5f5c8771c") {
                  ...
                  $("#navigationWrapper")
                    .find(".nextBtnQuestion")
                    .addClass("hidden");
                  $("#navigationWrapper")
                    .find(".submitQuestionnaire")
                    .removeClass("hidden");
                } else if (
                  choiceID == "f26cd3188710d81ef34664426f940" ||
                  choiceIDData[0] == "f26cd3188710d81ef34664426f940"
                ) {
                  progressBarMovement(count);
                  $("#navigationWrapper")
                    .find(".nextBtnQuestion")
            ...
            if (choiceIDData[0] !== "" && choiceIDData[0] == choiceID)
              choiceVal.attr("checked", true);
            ...
            const inputFieldWrapper = $("<div>").addClass(
              "flex justify-start items-center gap-2 w-1/2 p-2"
            );
            let name = questionTxt.replace(" ", "");
            const max = 1000;
            let id = choiceID + Math.floor(Math.random() * (max + 1));
      ...
      if (choiceIDData[0] == "ed7f8ed8d45c72df2d9d5f5c8771c") {
        // Have you ever been employed? No answer
    ...
    $.ajax({
      ...
      success: (response) => {
        ...
        $("#proceedBtnSection").on("click", function () {
          ...
          // check box checker
          const checkBoxes = $("#sectionQuestionContainer").find(
            ".checkBoxVal"
          );
          if (checkBoxes.length > 0) {
            // check if at least one checkbox is checked
    ...
    const unansweredRequiredQuestions = $('.questions [data-required="true"]')
      ...
      .filter((questionWrapper) => {
        if ($(questionWrapper).find('input[type="radio"]').length > 0) {
          const questionID = $(questionWrapper)
    ...
    $.ajax({
      ...
      success: (response) => {
        ...
        if (response == "Success") {
          ...
          $("#msgSuccess").text(
            "Thank you for the having time completing this Graduate Tracer Form."
          );
          ...
          setTimeout(() => {
            ...
            $("#msgSuccess").text(
              "Your feedback is important to us, and we take all reports seriously"
            );
