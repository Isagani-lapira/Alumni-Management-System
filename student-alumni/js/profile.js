$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: (response) => {
        if (response != "none") {
          ...
          for (let i = 0; i < length; i++) {
            ...
            imgProfile =
              imgProfile === ""
                ...
                : imgFormat + parsedResponse.profilePic[i];
            ...
            displayPost(
              ...
              isDeleted,
              isLiked
            ); //display the post on the container
          }
          ...
          $("#loadingData").addClass("hidden");
          $("#noProfPostMsg").removeClass("hidden").appendTo("#feedContainer");
        }
      },
      error: (error) => {
        console.log(error);
  ...
  $("#archievedBtn").on("click", function () {
    ...
    $("#userPost")
      .removeClass("text-white bg-accent font-bold")
  ...
  $("#userPost").on("click", function () {
    ...
    actionTracker = formDataAvailable;
    typeTracker = false;
    $("#loadingData").removeClass("hidden").appendTo("#feedContainer");
  ...
  function displayPost(
    ...
    const container = $("<div>").addClass(
      "containerPostProfile mx-2 flex gap-1 justify-center"
    );
    let postWrapper = $("<div>").addClass(
      "postWrapper mb-2 center-shadow w-full p-4 rounded-md"
    );
    ...
    // Markup for body
    let description = $("<p>")
      .addClass("text-sm text-gray-500 my-2 outline-none")
      .text(caption);
    ...
    if (images.length > 0) {
      swiperContainer = $("<div>").addClass(
        "swiper h-80 bg-black rounded-md cursor-pointer"
      );
      ...
      images.forEach((image) => {
        ...
        let slide = $("<div>").addClass(
          "swiper-slide relative flex justify-center items-center"
        );
      ...
      postWrapper.css("min-height", "100px").on("click", function () {
    ...
    editIcon.on("click", function () {
      ...
      description
        ...
        .focus()
        .addClass("border-b border-green-400");
      ...
      success: (response) => {
        ...
        if (parsedResponse.result == "Success") {
          ...
          for (let i = 0; i < length; i++) {
            ...
            const img =
              parsedResponse.profile[i] == ""
                ? "../assets/icons/person.png"
                : imgFormat + parsedResponse.profile[i];
            ...
            let delComment = $("<button>")
              .addClass("text-xs hover:text-red-400")
              .text("Delete")
              .on("click", function () {
  ...
  $("#saveTweet").on("click", function () {
    ...
    $("#editTwitter")
      .attr("disabled", true)
      .removeClass("border-b border-gray-400"); //back to normal
  ...
  $(".confirmEditBtn").on("click", function () {
    ...
    if (isReady) {
      ...
      checkAccountPass(currentPass).then((response) => {
        if (response === "successful") {
